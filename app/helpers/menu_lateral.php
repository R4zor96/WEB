<?php
    function configuracion_menu($dataSession, $pagina = '') {
        $menu = array();
            $menu_item = array();
                $sub_menu_item = array();
        ...
        $menu_item['icon'] = 'fas fa-tachometer-alt';
        $menu_item['text'] = 'Dashboard';
        $menu_item['submenu'] = array();
        $menu['dashboard'] = $menu_item;

        //ROL ADMINISTRADOR
        if($dataSession["id_rol"] == 128){
            //Opcion Usuarios
    ...
    function mostrar_menu_lateral($dataSession = array(), $pagina = ''){
       
        $menu = configuracion_menu($dataSession, $pagina);
        // print("<pre>".print_r($menu, true)."</pre>");
        $html = ' <li class="nav-header text-center">==== '.$_SESSION["rol"].' ====</li>';
